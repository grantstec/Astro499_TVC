...
public class GUI_GSEv2 extends PApplet {
...
public void connectToPort(int index) {
  try {
    if (port != null) {
...
public void drawSerialTerminal() {
  fill(50);
...
public void parseTelemetry(String data) {
  try {
    String[] values = split(trim(data), ','); // Split by commas
    if (values.length == 3) { // Expecting yaw, pitch, roll
      yaw = PApplet.parseFloat(split(values[0], ':')[1]);
      pitch = PApplet.parseFloat(split(values[1], ':')[1]);
      roll = PApplet.parseFloat(split(values[2], ':')[1]);
...
public void drawLineGraph(int x, int y, int w, int h, float[] data, int c, String label, float minValue, float maxValue) {
  stroke(c);
  noFill();
  rect(x, y, w, h);
...
public void displayTelemetry() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "GUI_GSEv2" };
    if (passedArgs != null) {
